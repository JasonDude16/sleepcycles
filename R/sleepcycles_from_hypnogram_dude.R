.sleepcycles_from_hypnogram_dude <- function(df, epoch_col, stage_col, options = list()) {
  ...
  df_sc <- .cycle_summary(
    ...
    stage_col = stage_col,
    options = options,
    cycle_type == "NREMP" & n_epochs > options$NREMP$min_size |
      cycle_type == "REMP" & n_epochs > options$REMP$min_size
  )
...
.check_sleepcycles_dude <- function(df, stage_col, options) {
  ...
  if (length(options) > 0) {
    if (!all(names(options) %in% valid_option_args)) {
      stop("all names in `options` list must be the following: ", paste0(valid_option_args, sep = ", "))
    }
    ...
    if ("combos" %in% names(options)) {
      if (is.null(names(options$combos))) {
        stop("`combos` should be a names listed.")
      }
      if (!all(unique(Reduce(c, options$combos)) %in% unique(df[[stage_col]]))) {
        stop("`combos` must be a list, and all list items must contain levels that are in `stage_col`")
      }
    }
    
    if ("kernel" %in% names(options)) {
      if (sum(options$kernel) != 1) {
        stop("The kernel should sum to 1. Please ensure it is normalized first.")
      }
      if (length(options$kernel) %% 2 != 1) {
        stop("The length of the kernel should be odd.")
      }
    }
    
    if ("NREMP" %in% names(options)) {
      if (!all(names(options$NREMP) %in% valid_nremp_remp_args)) {
        stop("Valid option names for `NREMP` are: ", paste0(valid_nremp_remp_args, sep = ", "))
      }
    }
    
    if ("REMP" %in% names(options)) {
      if (!all(names(options$REMP) %in% valid_nremp_remp_args)) {
        stop("Valid option names for `REMP` are: ", paste0(valid_nremp_remp_args, sep = ", "))
      }
    }
...
.convolve_with_padding <- function(signal, kernel) {
  ...
  signal_fft <- fft(c(signal, rep(0, N - length(signal))))
  kernel_fft <- fft(c(kernel, rep(0, N - length(kernel))))
  
  convolved_fft <- signal_fft * kernel_fft
  convolved_signal <- Re(fft(convolved_fft, inverse = TRUE)) / N
  ...
  for (i in seq_along(options$combos)) {
    res_combo_cols <- lapply(options$combos[[i]], function(s) {
      if (s %in% colnames(res) == FALSE) {
        warning("One of the combo levels (", s, ") is not present in hypnogram")
        return(NULL)
      } else {
        return(res[[s]])
      }
...
.cluster_segments <- function(df, epoch_col, stage_col, options) {
  ...
  if (any(missing_period)) {
    ...
    if (all(missing_period)) {
      warning("No NREMP or REMP periods were found, skipping...")
      return(NULL)
      
    }
    if (is.null(res[["NREMP"]])) {
      warning("No NREMP periods were found")
  
    }
    if (is.null(res[["REMP"]])) {
      warning("No REMP periods were found")
...
.handle_overlapping_cycles <- function(x) {
  ...
  for (r in 1:nrow(x)) {
    ...
    warning("NREMP and REMP overlap (", overlap[1], "-", overlap[length(overlap)], "). Splitting NREMP and keeping REMP...")
